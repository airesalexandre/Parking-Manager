...
The program accepts input as lines that start with a letter (the command) followed by the required arguments for that command. You can assume all inputs use the correct types (e.g., an integer field will never contain a letter). The table lists the available commands and the operations they perform.
